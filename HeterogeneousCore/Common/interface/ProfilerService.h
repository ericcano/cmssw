#ifndef __HeterogeneousCore_CUDAServices_ProfilerService_h__
#define __HeterogeneousCore_CUDAServices_ProfilerService_h__
...
#define DEFINE_ES_SIGNAL_WATCHER(signal) \
  ...
  void ProfilerService<Backend>::pre##signal(edm::eventsetup::EventSetupRecordKey const& iKey, edm::ESModuleCallingContext const& esmcc) { \
  ...
  template <class Backend> \
  void ProfilerService<Backend>::post##signal(edm::eventsetup::EventSetupRecordKey const& iKey,edm::ESModuleCallingContext const& esmcc) { \
    auto mid = esmcc.componentDescription()->id_; \
    auto const& label = esmcc.componentDescription()->label_; \
    auto const& type = esmcc.componentDescription()->type_; \
    std::string msg; \
...
#define REGISTER_SIGNAL_WATCHER(signal) \
  registry.watchPre##signal( \
      this, &ProfilerService::pre##signal); \
  registry.watchPost##signal( \
      this, &ProfilerService::post##signal);
...
private:
  ...
  std::vector<std::vector<Range>> stream_modules_acquire_;  // per-stream, per-module ranges for acquire, which can clash with produce
  ...
  std::vector<std::vector<Range>> stream_ES_modules_acquire_;  // per-stream, per-ES-module ranges for acquire, which can clash with produce
  // use a tbb::concurrent_vector rather than an std::vector because its final size is not known
...
ProfilerService<Backend>::ProfilerService(edm::ParameterSet const& config, edm::ActivityRegistry& registry)
  ...
  if (not service) {
    std::cout << Backend::shortName() << "ProfilerService: EDM service not available, disabling profiling service" << std::endl;
    return;
  }
  if (not service or not service->enabled()) {
    std::cout << Backend::shortName() << "ProfilerService: EDM service failed to be enabled, disabling profiling service" << std::endl;
    return;
  }
  std::cout << Backend::shortName() << "ProfilerService: EDM service initialized successfully. Registering watchers to EDM." << std::endl;
...
void ProfilerService<Backend>::preModuleConstruction(edm::ModuleDescription const& desc) {
  ...
  std::cout << "ProfilerService::preModuleConstruction: module id " << mid
...
template <class Backend>
void ProfilerService<Backend>::preModuleGlobalBeginRun(edm::GlobalContext const& gc, edm::ModuleCallingContext const& mcc) {
  if (not skipFirstEvent_ or globalFirstEventDone_) {
    auto mid = mcc.moduleDescription()->id();
    auto const& label = mcc.moduleDescription()->moduleLabel();
...
template <class Backend>
void ProfilerService<Backend>::postModuleGlobalBeginRun(edm::GlobalContext const& gc, edm::ModuleCallingContext const& mcc) {
  if (not skipFirstEvent_ or globalFirstEventDone_) {
    auto mid = mcc.moduleDescription()->id();
    auto const& label = mcc.moduleDescription()->moduleLabel();
    auto const& msg = label + " global begin run";
...
template <class Backend>
void ProfilerService<Backend>::preModuleGlobalEndRun(edm::GlobalContext const& gc, edm::ModuleCallingContext const& mcc) {
  if (not skipFirstEvent_ or globalFirstEventDone_) {
    auto mid = mcc.moduleDescription()->id();
    auto const& label = mcc.moduleDescription()->moduleLabel();
...
template <class Backend>
void ProfilerService<Backend>::postModuleGlobalEndRun(edm::GlobalContext const& gc, edm::ModuleCallingContext const& mcc) {
  if (not skipFirstEvent_ or globalFirstEventDone_) {
    auto mid = mcc.moduleDescription()->id();
    auto const& label = mcc.moduleDescription()->moduleLabel();
    auto const& msg = label + " global end run";
...
template <class Backend>
void ProfilerService<Backend>::preModuleGlobalBeginLumi(edm::GlobalContext const& gc, edm::ModuleCallingContext const& mcc) {
  if (not skipFirstEvent_ or globalFirstEventDone_) {
    auto mid = mcc.moduleDescription()->id();
    auto const& label = mcc.moduleDescription()->moduleLabel();
...
template <class Backend>
void ProfilerService<Backend>::postModuleGlobalBeginLumi(edm::GlobalContext const& gc, edm::ModuleCallingContext const& mcc) {
  if (not skipFirstEvent_ or globalFirstEventDone_) {
    auto mid = mcc.moduleDescription()->id();
    auto const& label = mcc.moduleDescription()->moduleLabel();
    auto const& msg = label + " global begin lumi";
...
template <class Backend>
void ProfilerService<Backend>::preModuleGlobalEndLumi(edm::GlobalContext const& gc, edm::ModuleCallingContext const& mcc) {
  if (not skipFirstEvent_ or globalFirstEventDone_) {
    auto mid = mcc.moduleDescription()->id();
    auto const& label = mcc.moduleDescription()->moduleLabel();
...
template <class Backend>
void ProfilerService<Backend>::postModuleGlobalEndLumi(edm::GlobalContext const& gc, edm::ModuleCallingContext const& mcc) {
  if (not skipFirstEvent_ or globalFirstEventDone_) {
    auto mid = mcc.moduleDescription()->id();
    auto const& label = mcc.moduleDescription()->moduleLabel();
    auto const& msg = label + " global end lumi";
...
template <class Backend>
void ProfilerService<Backend>::postESModuleRegistration(edm::eventsetup::ComponentDescription const& componentDescription) {
...
template <class Backend>
void ProfilerService<Backend>::preESModulePrefetching(edm::eventsetup::EventSetupRecordKey const& iKey, edm::ESModuleCallingContext const& esmcc) { 
  auto mid = esmcc.componentDescription()->id_; 
  auto const& label = esmcc.componentDescription()->label_;
  auto const& type = esmcc.componentDescription()->type_;
  std::string msg;
...
template <class Backend>
void ProfilerService<Backend>::postESModulePrefetching(edm::eventsetup::EventSetupRecordKey const& iKey,edm::ESModuleCallingContext const& esmcc) { 
  auto mid = esmcc.componentDescription()->id_;
  auto const& label = esmcc.componentDescription()->label_;
  auto const& type = esmcc.componentDescription()->type_;
  std::string msg;
...
#endif  // __HeterogeneousCore_CUDAServices_ProfilerService_h__