...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  void TestAlgo::fillMulti(Queue& queue, portabletest::TestDeviceMultiCollection& collection, double xvalue) const {
    // use 64 items per group (this value is arbitrary, but it's a reasonable starting point)
    uint32_t items = 64;
    ...
    alpaka::exec<Acc1D>(queue, workDiv2, TestAlgoMultiKernel{}, collection.view<portabletest::TestSoA2>(), xvalue);
  }
  ...
  void TestAlgo::fillMulti3(Queue& queue, portabletest::TestDeviceMultiCollection3& collection, double xvalue) const {
    ...
    uint32_t groups = divide_up_by(collection->metadata().size(), items);
    uint32_t groups2 = divide_up_by(collection.view<1>().metadata().size(), items);
    uint32_t groups3 = divide_up_by(collection.view<2>().metadata().size(), items);
