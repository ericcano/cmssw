...
const std::pair<float, float> nonBlockScatter(MPIData& mpiInput) {
  ...
  if (mpiInput.rank)  //Only for Workers
  {
    ...
    addVectorsGpu<<<blocks,threads>>> (d_vect1, d_vect2, d_vect3Gpu, mpiInput.vectorWorkers1.size()); //call device function to add two vectors and save into vect3Gpu.
    
    cudaMemcpy(mpiInput.vectorWorkers1.data(), d_vect3Gpu, mpiInput.vectorWorkers1.size(), cudaMemcpyDeviceToHost);
  }
