...
void checkContents(Hist *h, unsigned int N, VERIFY & verify, INCR & incr, 
        T window, uint32_t nParts, const T* v, const uint32_t* offsets) {
...
int go(const DevHost& host, const Device& device, Queue& queue) {
  ...
  std::cout << "HistoContainer Runtime sized " << (int)(offsetof(HistR, off)) << ' ' << HistR::nbins() << ' ' << HistR::totbins() << ' '
            << HistR{}.capacity() << ' ' << offsetof(HistR, content) - offsetof(HistR, off) << ' '
