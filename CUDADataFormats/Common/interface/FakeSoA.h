...
#define myassert(A) if (!(A)) { std::cerr << "Failed assertion: " #A " at  " __FILE__ "("  << __LINE__ << ")" << std::endl; abort(); }
...
public:
  FakeSoA(): size_(0) {
    std::cout << "At end of FakeSoA::FakeSoA()" << std::endl;
  }
  ...
  void Dump() {
    ...
    << " buffer size=" << computeBufferSize(size_) << "(" << std::hex << computeBufferSize(size_)  << ")" << std::endl;
  }
  
  void DumpData() {
    std::cout << "a16_[0]=" << a16_[0] << " b32_[0]=" << b32_[0] << std::endl;
  }
  ...
  void Check() {
    for (Int_t i=0; i<size_; i++) {
      if(a16_[i] != 42 + i) { std::cout << "a16 mismatch at i=" << i << "(" << a16_[i] << "/" << 42 + i << ")" << std::endl; }
      if(b32_[i] != 24 + (uint32_t)i) { std::cout << "b32 mismatch at i=" << i << "(" << b32_[i] << "/" << 24 + i << ")" << std::endl; }
    }
  }
