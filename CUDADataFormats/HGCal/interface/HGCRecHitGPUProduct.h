...
public:
  ...
  explicit HGCRecHitGPUProduct(uint32_t nhits, const cudaStream_t &stream) : nhits_(nhits) {
    ...
    mMemRecHitDev = cms::cuda::make_device_unique<std::byte[]>(pad_ * size_tot_, stream);
  }
  ~HGCRecHitGPUProduct() = default;
  ...
  HGCRecHitSoA get() {
    HGCRecHitSoA soa;
    soa.energy = reinterpret_cast<float *>(mMemRecHitDev.get());
  ...
  ConstHGCRecHitSoA get() const {
    ...
    soa.energy = reinterpret_cast<float const *>(mMemRecHitDev.get());
    soa.time = soa.energy + pad_;
    soa.timeError = soa.time + pad_;
    soa.sigmaNoise = soa.timeError + pad_;
...
private:
  cms::cuda::device::unique_ptr<std::byte[]> mMemRecHitDev;
